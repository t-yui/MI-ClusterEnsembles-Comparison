...
seed_num = 1
kpod_script = "realdata_support_kpod.R"
...
def run_kpod_clustering(selected_k):
    selected_k_path = f"{output_dir}/support_selected_k.txt"
    with open(selected_k_path, "w") as f:
        f.write(f"{selected_k}\n")
    subprocess.run(["Rscript", kpod_script], check=True)

    labels_df = pd.read_csv(f"{output_dir}/support_kpod_labels.csv")
    completed_df = pd.read_csv(f"{output_dir}/support_kpod_completed_scaled.csv")
...
if k is None:
    ...
    res_kselect.to_csv(f"{output_dir}/support_k_details.csv", index=False)
    ...
    res_kselect_summary.to_csv(
        f"{output_dir}/support_k_selection_summary.csv",
        index=False,
    )
    ...
    plot_k_selection_elbow(
        res_kselect_summary,
        selected_k,
        f"{figures_dir}/support_k_selection_elbow",
    )

else:
    selected_k = int(k)

print(f"Selected k for SUPPORT = {selected_k}")
...
base_clusterings.reset_index().to_csv(f"{output_dir}/support_base_clusterings.csv", index=False)
...
labels_df.reset_index().to_csv(f"{output_dir}/support_consensus_labels.csv", index=False)
...
res_scores.to_csv(f"{output_dir}/support_internal_values.csv", index=False)
...
method_summary.to_csv(f"{output_dir}/support_method_summary.csv", index=False)
...
res_cluster_sizes.to_csv(f"{output_dir}/support_cluster_sizes.csv", index=False)


# method agreement
agreement_matrix = make_agreement_matrix(labels_dict)
agreement_matrix.to_csv(f"{output_dir}/support_method_agreement.csv")


# merged output
res_merged = pd.merge(meta_df, labels_df.reset_index(), on="id", how="left")
res_merged.to_csv(f"{output_dir}/support_clusters_with_meta.csv", index=False)
