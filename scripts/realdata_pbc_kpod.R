...
k <- 6
maxiter <- 100
selected_k_path <- file.path(output_dir, "pbc_selected_k.txt")
...
# load data
X <- read.csv(file.path(input_dir, "pbc_missing_data.csv"))
ids <- X[, 1]
X <- X[, -1]
X <- scale(X)
...
write.csv(
  ...
  file.path(output_dir, "pbc_kpod_labels.csv"),
  row.names = FALSE
)
write.csv(
  cbind(id = ids, X_completed),
  file.path(output_dir, "pbc_kpod_completed_scaled.csv"),
  row.names = FALSE
)

cat("Processed: PBC k-pod, k =", k, "\n")
